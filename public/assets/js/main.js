(function ($) {
	...
	$(".geex-sidebar__menu__link").click(function(e) {
		let $clickedItem = $(this);
		let $submenu = $clickedItem.siblings('.geex-sidebar__submenu');

        // Only handle submenu toggling if the clicked item has a submenu
        if ($submenu.length > 0) {
            e.preventDefault(); // Prevent navigation only if there's a submenu
