...
class ChatbotController extends BaseController
{
    ...
    /**
     * Generate chatbot response with intelligent search system
     */
    public function generateResponse()
    {
        try {
            ...
            // INTELLIGENT SEARCH SYSTEM: FAQ -> AI Response
            
            // Log the search for debugging
            log_message('info', 'Chatbot searching for: ' . $message);
            ...
            if (!empty($faqResult)) {
                ...
                log_message('info', 'No FAQ match found for: ' . $message);
            }
            
            // Step 2: Check if this is a product search query
            log_message('info', 'Checking if message is product search: ' . $message);
            if ($this->isProductSearch($message)) {
                ...
                if (!empty($products)) {
                    ...
                    log_message('info', 'No products found for search query: ' . $message);
                }
            } else {
                log_message('info', 'Message is not a product search query: ' . $message);
            }
            ...
            log_message('info', 'Google AI response received: ' . ($aiResponse ? 'YES' : 'NO') . ' - Length: ' . ($aiResponse ? strlen($aiResponse) : 0));
            
            if ($aiResponse && !empty(trim($aiResponse))) {
                if ($aiResponse === 'RATE_LIMIT_EXCEEDED') {
                    log_message('info', 'Google AI rate limit exceeded, skipping to Hugging Face');
                } else {
                ...
                log_message('warning', 'Google AI response is empty or null, falling back to Hugging Face');
            }
            ...
            if ($huggingFaceResponse && !empty(trim($huggingFaceResponse))) {
                ...
                log_message('warning', 'Hugging Face also failed, using smart fallback');
            }
